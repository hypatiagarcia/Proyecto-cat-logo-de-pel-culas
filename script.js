...
async function fetchPopularMovies() {
    try {
        const respuesta = await fetch(`${apiUrl}/movie/popular?api_key=${apiKey}&language=es-ES`)
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        try {
            const respuesta_boton = await fetch(`${apiUrl}/search/movie?api_key=${apiKey}&language=es-ES&query=${encodeURIComponent(query)}`)
            const data = await respuesta_boton.json()
            displayMovies(data.results)
        } catch (error) {
